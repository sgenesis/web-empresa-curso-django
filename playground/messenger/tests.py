...
class ThreadTestCase(TestCase):
    ...
    def test_add_messeges_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Que onda")
        message2 = Message.objects.create(user=self.user2, content="Que pues carnal")
    ...
    def test_add_messeges_from_user_not_in_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Que onda")
        message2 = Message.objects.create(user=self.user2, content="Que pues carnal")
        message3 = Message.objects.create(user=self.user3, content="Aquí nomas ")
